...
-}
modify : ⦃ d : is-discrete A ⦄ {Γ : LFSet A}
       → A → Bool → Val Γ → Val Γ
modify x a v y = if unvar y =? x then a else v y

on-all-vals : ⦃ d : is-discrete A ⦄ {Γ : LFSet A}
            → (Val Γ → B)
            → (B → B → B)
            → Val Γ → List A → B
...
tautology : ⦃ d : is-discrete A ⦄ {Γ : LFSet A}
          → Formulaᵢ Γ → Bool
tautology f = on-all-vals (eval f) _and_ (λ _ → false) (map unvar $ atomsᵢ f)

-- satisfied by no valuation
unsatisfiable : ⦃ d : is-discrete A ⦄ {Γ : LFSet A}
             → Formulaᵢ Γ → Bool
unsatisfiable = tautology ∘ Not

-- satisfied by some valuation(s)
satisfiable : ⦃ d : is-discrete A ⦄ {Γ : LFSet A}
            → Formulaᵢ Γ → Bool
